...
$MyQuery= "SELECT
    ...
    db_invoice_g.invoice_taxes,
    db_clientes.clientes_phone,
    db_invoice_g.invoice_amount,
    db_invoice_g.invoice_taxes_total,
    ...
    WHERE db_invoice_g.invoice_taxes = 'Y' AND (DATE(db_invoice_g.invoice_fecha)>='$_POST[desdeSubmit]' AND DATE(db_invoice_g.invoice_fecha)<='$_POST[cuandoSubmit]')";
...
class MYPDF extends TCPDF 
{
	public function Header() 
	{
		...
		$this->Cell(0,10,'Invoices/Taxes Report',0,1,'C',0);
        $this->SetX(10);
...
$pdf->Cell(20,6,"Phone",1,0,'C',1);
$pdf->Cell(30,6,"Date",1,0,'C',1);
$pdf->Cell(20,6,"Amount",1,0,'C',1);
$pdf->Cell(20,6,"Taxes",1,1,'C',1);
$pdf->SetFont('times','',8);
$contador=1;
$lineas=$total=$taxes=0;
while ($fila = mysqli_fetch_assoc($tabla)) 
{
	...
	$pdf->Cell(20,6,$fila['clientes_phone'],1,0,'C',0);
	$pdf->Cell(30,6,$fila['Invoice_Fecha'],1,0,'C',0);
	$pdf->Cell(20,6,$fila['MontoInvoice'],1,0,'R',0);
	$pdf->Cell(20,6,$fila['invoice_taxes_total'],1,1,'R',0);
    $total = $total + $fila['invoice_amount'];
    $taxes = $taxes + $fila['invoice_taxes_total'];
	if(($contador==30) and ($registros>1))
	{
		$pdf->AddPage();
        $pdf->SetFillColor(220,220,220);
        $pdf->SetTextColor(0,0,0);
...
$pdf->Cell(60,5,"Total Invoices: $".number_format($total,2),0,1,'L',0);
$pdf->Cell(60,5,"Total Taxes: $".number_format($taxes,2),0,1,'L',0);
$pdf->Output(DIR_TEMP . 'list_taxes.pdf', 'F');
$pdf->Close();
?>